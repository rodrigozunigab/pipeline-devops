def call(stageOptions){
        ...
        stage("Run"){
            env.TAREA =  env.STAGE_NAME 
            if ((stageOptions.contains('Run') || (stageOptions =='')) && (env.EJCUTARBUILD) ){ 
        ...
        stage("Rest"){
            env.TAREA =  env.STAGE_NAME 
            if ((stageOptions.contains('Rest') || (stageOptions =='')) && (env.EJCUTARBUILD) ) 
                sh 'curl -X GET "http://localhost:8081/rest/mscovid/test?msg=testing"'
        }  
        stage("Nexus"){    
            env.TAREA =  env.STAGE_NAME   
            if ((stageOptions.contains('Nexus') || (stageOptions =='')) && (env.EJCUTARBUILD) )      
